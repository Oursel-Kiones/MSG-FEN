print("--- SCRIPT VERSION CHECK: THIS IS THE LATEST MODIFIED FILE (V2.8 - FULL UNABRIDGED + SHAPE ALIGNMENT) ---")
# -*- coding: utf-8 -*-
"""
panoptic_deeplab_trainer.py (Version 2.8)

An advanced, robust, and feature-rich training script for DeepLab-based models.
V2.8 incorporates CRITICAL bug fixes for MSG-FENet Stage 1 while keeping ALL original code intact:
1. Fixed variable image sizes using Dynamic Padding (pad_and_stack).
2. Fixed the extra dimension bug by using view_as() for perfect target/input alignment.
3. Removed the catastrophic unbatched loop to save BatchNorm.
4. Default batch size adjusted to 2 to prevent OOM.
"""
import argparse
import os
import numpy as np
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.training_stage == 1:
            print("INFO: Configuring optimizer for [Stage 1] training (Full Network).")
    ...
    # =========================================================================
    # 【V2.8 核心修复】：动态 Padding 函数，解决尺寸不一导致无法组成 Batch 的问题
    # =========================================================================
    def _pad_and_stack(self, tensor_list, pad_value=0):
        """
        Dynamically pads a list of tensors to the max height and width in the batch,
        then stacks them.
        """
        if not isinstance(tensor_list, list):
            return tensor_list
            
        # 找到当前 batch 中的最大高度和宽度
        ...
        for t in tensor_list:
            ...
            if pad_h > 0 or pad_w > 0:
                # F.pad 的顺序是 (左, 右, 上, 下)
                # 我们只需要在右侧和下侧填充
                padded = F.pad(t, (0, pad_w, 0, pad_h), value=pad_value)
            else:
                padded = t
    ...
    def training(self, epoch):
        ...
        for i, sample_batch in enumerate(tbar):
            ...
            if self.args.training_stage == 1:
                # 【应用动态填充】：真值标签空白处填充 255 (Ignore Index)，防止影响 Loss
                stuff_gt = self._pad_and_stack(sample_batch['stuff_gt'], pad_value=255).cuda()
                objectness_gt = self._pad_and_stack(sample_batch['objectness_gt'], pad_value=255).cuda()
                semantic_gt = self._pad_and_stack(sample_batch['label'], pad_value=255).long().cuda()

                pred_stuff, pred_objectness = predictions['stuff'], predictions['objectness']
                
                # 【终极防弹修复】：使用 view_as 强制对齐维度，彻底解决 [2,1,1,...] 的问题
                objectness_gt = objectness_gt.view_as(pred_objectness)
                
                loss_stuff = self.criterion.L_Stuff_CE(pred_stuff, stuff_gt)
                
                valid_mask = (semantic_gt != self.criterion.ignore_index)
                
                if not valid_mask.any() and i % 100 == 0:
                    print(f"\nWarning: Batch {i} has no valid pixels.")
                ...
                final_loss = (self.args.w_stuff * loss_stuff + self.args.w_coarse_obj * loss_objectness)

            else: # End-to-end
                target = self._pad_and_stack(sample_batch['label'], pad_value=255).long().cuda()
                pred_semantic = predictions.get('semantic', predictions)
                loss_fn = self.criterion.build_loss(mode=self.args.loss_type)
                final_loss = loss_fn(pred_semantic, target)
    ...
    def visualize_stage1_outputs(self, sample: Dict, predictions: Dict, epoch: int):
        img_tensor = self._pad_and_stack(sample['image'], pad_value=0.0)[0].cpu()
        stuff_gt = self._pad_and_stack(sample['stuff_gt'], pad_value=255)[0].cpu().numpy()
        objectness_gt = self._pad_and_stack(sample['objectness_gt'], pad_value=255)[0].squeeze().cpu().numpy()
        
        pred_stuff_logits = predictions['stuff']
        pred_objectness_logits = predictions['objectness']

        pred_stuff = torch.argmax(pred_stuff_logits, dim=1)[0].cpu().numpy()
        pred_objectness = torch.sigmoid(pred_objectness_logits)[0].squeeze().cpu().numpy()
    ...
    def validation(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            target_batch = self._pad_and_stack(sample.get('label'), pad_value=255) if sample.get('label') is not None else None
            
            if self.args.cuda:
                image_batch = image_batch.cuda()
                if target_batch is not None: target_batch = target_batch.cuda()

            with torch.no_grad():
                predictions = self.model(image_batch)

                if self.args.training_stage == 1:
                    stuff_gt = self._pad_and_stack(sample['stuff_gt'], pad_value=255).cuda()
                    objectness_gt = self._pad_and_stack(sample['objectness_gt'], pad_value=255).cuda()
                    semantic_gt = self._pad_and_stack(sample['label'], pad_value=255).long().cuda()
                    
                    pred_stuff, pred_objectness = predictions['stuff'], predictions['objectness']
                    
                    # 【终极防弹修复】：同样在验证集使用 view_as 强制对齐
                    objectness_gt = objectness_gt.view_as(pred_objectness)
            ...
            if self.args.save_val_results and i == 0:
                if self.args.training_stage == 1:
                    with torch.no_grad():
                        single_image = self._pad_and_stack(sample['image'], pad_value=0.0)[0].unsqueeze(0).cuda()
                ...
                if predictions.get('semantic') is not None and target_batch is not None:
                    ...
                    mean, std = np.array([0.485, 0.456, 0.406]), np.array([0.229, 0.224, 0.225])
                    img_tmp = np.clip((self._pad_and_stack(sample['image'], pad_value=0.0)[0].cpu().numpy().transpose(1, 2, 0) * std + mean) * 255.0, 0, 255).astype(np.uint8)
...
def main():
    parser = argparse.ArgumentParser(description="Advanced PyTorch DeeplabV3Plus Multi-Stage Trainer (V2.8)")
